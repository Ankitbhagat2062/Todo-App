  ...
  const handleSaveEvent = async (e) => {
    ...
    if (userId) {
      try {
        ...
        if (!response.ok) {
         toast("Error Saving Event Data to the server");
        }
        if (response.ok) {
          ...
          toast("Event Saved Successfully in server")
        ...
        toast("Failed to connect with Database Try aggain later");
      }
    }
   (!userId) && toast("Your Event is not saved to server Please Signup");
  };
    ...
    if (selectedEvent) {
      ...
      if (userId) {
        try {
          ...
          if (response.ok) {
            toast('Event deleted successfully both from server and web');
          } else {
            toast('Failed to delete item.');
          }
        } catch {
          toast('Failed to connect with Backend server');
        }
      }  
    (!userId) && toast('Please Sign up/login.Your Event has not been deleted');
    }
  };
      ...
      await fetch(`${API_BASE_URL}/api/v3/updateEvent/${EventId}`, {
        ...
        if (!response.ok) {
          ...
          if (errorData.message === "User not found") {
            toast("There is no such user in our database. Please Sign up.");
          }
        }
        if (response.ok) {
          toast('Your Event has Updated successfully');
          setEventsState(prev => prev.map(event => event.id === EditForm.id ? updatedEvent : event));
          setAddingEvent(false);
          setSelectedEvent(null)
          setShowEditForm(false)
